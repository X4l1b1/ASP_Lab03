...
ulong get_pixel_add (ulong X, ulong Y)
  {
	  return (Y * PIXELS_PER_ROW + X) * 2;  // décalage de 2 car 1 pixel = 2 octets dans le fram buffer
  }
	...
	for(i = 0; i < FONT_HEIGHT; ++i)
	{
		for(j = 0; j < FONT_WIDTH; ++j)
		{
			if(fb_font_data[cchar][i] & (0x80 >> j))
				...
				fb_set_pixel(x + j, y + i, lcd_bg_color);
		}
	}
}
...
void fb_print_string(uchar *pcbuffer, ulong x, ulong y, uchar color)
{
   ...
   // les \n sont traités comme les \r\n, mais les \r restent des \r
   // Caractère "opaques" (avec couleur de fond)
   ...
   for(i = 0; *(pcbuffer+i) != '\0'; ++i){
	   ...
	   else if(*(pcbuffer+i) == '\r'){
		   x = 0;
	   }
	   else if(*(pcbuffer+i) == '\t'){
		   x += N_SPACE_TAB * FONT_WIDTH;
	   }
	   else{
