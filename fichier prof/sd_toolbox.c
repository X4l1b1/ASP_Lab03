...
//	change_frequency((ulong) MMCHS_SYSCTL_CLKD_DIV480);
	...
	// Send CMD10, to get the card identification register
	mmchs_send_command((ulong) 10, arg, 0, 0);
	// response (type R2) is the CID register, copy it in a global variable
	...
	// get CSD register
	// Send CMD9, to get the card specific data (CSD register)
	mmchs_send_command((ulong) 9, arg, 0, 0);
	// response (type R2) is the CSD register, copy it in a global variable
...
int mmchs_write_block(const vulong *data, ulong block)
{
	...
	// disable buffer read flag
	MMCHS1_REG(MMCHS_IE) &= ~MMCHS_IE_BRR_ENABLE;
	// enable buffer write flag
	...
	// Send CMD24 command: write block
	arg=block;
	...
	{
	// write next 4 bytes to the buffer
		MMCHS1_REG(MMCHS_DATA) = *data;
		data++;
	}
...
int mmchs_write_multiple_block(const uchar *data, ulong block, uchar nblocks)
{
	...
	if ((nblocks==0)|| (nblocks>8)) return 1;
	...
	/* write multiple blocks (between 2 and 8) */
	...
	// Send CMD25 command: write multiple block
	arg=block;
	...
	data_ptr = (ulong *)buffer_data;

	ulong limit=(SD_BLOCK_LENGTH*nblocks)/4;


	for (k=0;k<limit;k++)
	{
		if((k%(SD_BLOCK_LENGTH/4))== 0)
		{
			// wait for buffer write ready
			while ((MMCHS1_REG(MMCHS_STAT) & MMCHS_STAT_BWR) == MMCHS_STAT_BWR_NOTREADY);
			// clear flag
	...
	// send CMD12 stop command (CMD23 not supported by our SD card)
	arg=0;
	mmchs_send_command((ulong) 12, arg, 0, 0);
...
int mmchs_read_multiple_block(uchar *data, ulong block, uchar nblocks)
{
	...
	/* read multiple blocks (between 2 and 8) */
	...
	// copy data to the destination address passed as parameter
	memcpy(data,(void *)buffer_data,SD_BLOCK_LENGTH*nblocks);
...
ulong read_card_size()
{
	...
	//Completer le code ici
	if(csd_reg[2]!=0 || csd_reg[1]!=0)
	{
		size = (0x003F & csd_reg[2])<<16 | csd_reg[1]>>16;
		//size = SD_CSD1_DEV_SIZE(csd_reg[2], csd_reg[1]);
